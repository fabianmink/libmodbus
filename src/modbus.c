...
/* Write the values from the array to the registers of the remote device. This
 * is a non-standard function used by several chinese devices.  */
int modbus_write_registers_fc06(modbus_t *ctx, int addr, int nb, const uint16_t *src)
{
    int rc;
    int i;
    int req_length;
    ...
    req_length = ctx->backend->build_request_basis(ctx,
                                                   MODBUS_FC_WRITE_SINGLE_REGISTER, //THIS IS NON-STANDARD!!
                                                   addr, nb, req);
    byte_count = nb * 2;
    req[req_length++] = byte_count;
